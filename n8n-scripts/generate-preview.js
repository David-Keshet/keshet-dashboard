...
const data = {
  ...
  price_with_vat: 74,
  price_per_unit: null,
  tip: 'לשמשונית מומלץ ציפוי למינציה לעמידות מרבית בחוץ.'
};
...
const bodyContent = `
  ...
  <section class="mt-10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      ${data.quantity ? `<div class="flex gap-4 p-6 rounded-3xl bg-slate-50 border border-slate-100">
        <div class="w-12 h-12 rounded-2xl bg-white shadow-sm flex items-center justify-center"><span style="font-size:22px">🔢</span></div>
        <div>
          <div class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">כמות</div>
          <div class="text-base font-bold text-slate-800">${data.quantity ? data.quantity.toLocaleString('he-IL') + ' יחידות' : ''}</div>
        </div>
      </div>` : ''}
      ${data.size ? `<div class="flex gap-4 p-6 rounded-3xl bg-slate-50 border border-slate-100">
        <div class="w-12 h-12 rounded-2xl bg-white shadow-sm flex items-center justify-center"><span style="font-size:22px">📐</span></div>
        <div>
      ...
      ${data.print_type ? `<div class="flex gap-4 p-6 rounded-3xl bg-slate-50 border border-slate-100">
      ...
      ${data.paper ? `<div class="flex gap-4 p-6 rounded-3xl bg-slate-50 border border-slate-100">
        <div class="w-12 h-12 rounded-2xl bg-white shadow-sm flex items-center justify-center"><span style="font-size:22px">📄</span></div>
        <div>
